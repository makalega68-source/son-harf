...
'''                    Surface(shape = RoundedCornerShape(13.dp), color = Color(0xFFF7F9FC)) {
                        if (lastDisplay.isBlank()) {
                            Text(sh("İlk kelimeyi sen başlat", "You start the first word"), Modifier.padding(horizontal = 12.dp, vertical = 7.dp), color = LMuted, fontSize = 13.sp, fontWeight = FontWeight.Bold)
                        } else {
                            Column(Modifier.padding(horizontal = 12.dp, vertical = 6.dp), horizontalAlignment = Alignment.CenterHorizontally) {
                                ...
                                Text(lastDisplay, color = LText, fontSize = 16.sp, lineHeight = 18.sp, fontWeight = FontWeight.Black, maxLines = 1, overflow = TextOverflow.Ellipsis)
                            }
                        }
                    }
''', "last accepted word emphasis")

patch(light, "                        fontSize = 78.sp,\n                        lineHeight = 80.sp,\n", "                        fontSize = 62.sp,\n                        lineHeight = 64.sp,\n", "required letter size")
patch(light,
'''                        color = LBlue,
                        fontSize = 13.sp,
...
''', "required letter guidance")

patch(light, "        modifier = modifier.heightIn(min = 112.dp),\n", "        modifier = modifier.height(96.dp),\n", "equal player card height")
patch(light,
...
''', "", "repeated input status removal")

# Scope the first fillMaxWidth replacement to CompetitiveInputBar.
p = Path(light)
t = p.read_text(encoding="utf-8")
anchor = '''private fun CompetitiveInputBar('''
pos = t.index(anchor)
needle = '''        modifier = modifier.fillMaxWidth(),\n'''
idx = t.index(needle, pos)
t = t[:idx] + '''        modifier = modifier.fillMaxWidth().height(52.dp),\n''' + t[idx + len(needle):]
...
'''        Row(Modifier.fillMaxSize().padding(horizontal = 5.dp), verticalAlignment = Alignment.CenterVertically) {
            IconButton(onClick = onVoice, enabled = myTurn && !busy && !quiz && voiceSupported && voiceUses < 5, modifier = Modifier.size(48.dp)) {
                Surface(modifier = Modifier.size(28.dp), shape = CircleShape, color = if (voiceSupported && voiceUses < 5) LBlueSoft else LCard2, border = BorderStroke(1.dp, LBlue.copy(alpha = .24f))) {
                    Box(contentAlignment = Alignment.Center) {
                        Text("🎙${(5 - voiceUses).coerceAtLeast(0)}", color = LBlue, fontSize = 8.sp, fontWeight = FontWeight.Black, maxLines = 1)
                    }
                }
            }
            Spacer(Modifier.width(2.dp))
            Text(value, color = LText, fontSize = if (value.isBlank()) 16.sp else 20.sp, fontWeight = FontWeight.Black, modifier = Modifier.weight(1f), maxLines = 1, overflow = TextOverflow.Ellipsis)
        }
''', "compact input without placeholder or mini send")
...
'''private fun SmallAction(label: String, accent: Color, modifier: Modifier, onClick: () -> Unit) {
    Box(modifier = modifier.height(48.dp).clickable(onClick = onClick), contentAlignment = Alignment.Center) {
        Surface(modifier = Modifier.fillMaxWidth().height(34.dp), shape = RoundedCornerShape(11.dp), color = Color.Transparent, border = BorderStroke(1.dp, accent.copy(alpha = .4f))) {
            Box(contentAlignment = Alignment.Center) { Text(label, color = accent, fontSize = 8.sp, fontWeight = FontWeight.Black, maxLines = 1) }
        }
    }
}
''', "compact action visuals with 48dp touch")
...
''', "remove dictionary success box")

# Restrict modifier replacement to score-card function.
p = Path(siege)
t = p.read_text(encoding="utf-8")
pos = t.index("private fun WordSiegePracticeScoreCard(")
needle = "        modifier = modifier,\n"
idx = t.index(needle, pos)
t = t[:idx] + "        modifier = modifier.height(if (compact) 70.dp else 78.dp),\n" + t[idx + len(needle):]
p.write_text(t, encoding="utf-8")
...
''',
'''                    Text(name, color = MainUi.Text, fontWeight = FontWeight.Black, fontSize = 15.sp, lineHeight = 17.sp, maxLines = 1, overflow = TextOverflow.Ellipsis, modifier = Modifier.weight(1f))
''', "siege name fit")

patch(board,
'''    var closeScale by remember { mutableFloatStateOf(WORD_SIEGE_PRACTICE_CLOSE_SCALE) }
''',
'''    var closeScale by remember { mutableFloatStateOf(0.81f) }
''', "siege initial scale")

print("active UI patch complete")
