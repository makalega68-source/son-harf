...
Deno.serve(async (req: Request) => {
  ...
  if (isSubscription) {
    ...
    if (!entitledSubscriptionStates.has(playState)) return response(409, { error: "subscription_not_entitled", state: playState });
  ...
  if (consumableProducts.has(productId)) {
    ...
    const consumeResponse = await fetch(consumeUrl, { method: "POST", headers: { Authorization: `Bearer ${access.token}`, "Content-Type": "application/json" }, body: JSON.stringify({}) });
    consumed = consumeResponse.ok;
    acknowledged = acknowledged || consumed;
    if (!consumed) return response(502, { error: "google_play_consume_failed", verified: true, productId, retryable: true });
    ...
    const ackResponse = await fetch(ackUrl, { method: "POST", headers: { Authorization: `Bearer ${access.token}`, "Content-Type": "application/json" }, body: JSON.stringify({}) });
