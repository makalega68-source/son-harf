...
returns boolean language sql stable security definer set search_path=public as $$
  ...
  or exists(select 1 from public.profile_photo_access a where a.owner_id=p_owner and a.viewer_id=p_viewer and a.allowed=true)
  or exists(
    select 1 from public.profiles p
    where p.id=p_owner and (
      p.avatar_visibility='public'
      or (p.avatar_visibility='vip' and exists(select 1 from public.profiles v where v.id=p_viewer and v.is_vip=true))
...
-- Owner can upload/update/delete only inside own folder.
...
-- Read is governed by the profile privacy model; the first path segment is the owner UUID.
