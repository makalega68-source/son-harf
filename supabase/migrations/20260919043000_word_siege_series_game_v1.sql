-- Premium Seri Oyun for Kelime Kuşatması.
-- Separate pool, minute turns, automatic timeout pass, third consecutive miss loses.
...
update public.word_siege_games set game_mode='classic' where game_mode is null or game_mode='';

alter table public.word_siege_games drop constraint if exists word_siege_games_game_mode_v1_check;
alter table public.word_siege_games drop constraint if exists word_siege_games_series_minutes_v1_check;
alter table public.word_siege_games drop constraint if exists word_siege_games_p1_missed_v1_check;
alter table public.word_siege_games drop constraint if exists word_siege_games_p2_missed_v1_check;
alter table public.word_siege_games
  add constraint word_siege_games_game_mode_v1_check check (game_mode in ('classic','series')),
  add constraint word_siege_games_series_minutes_v1_check check (turn_duration_minutes is null or turn_duration_minutes in (3,5,10)),
...
-- Classic matchmaking must never consume a Series waiting row.
...
begin
  ...
  update public.word_siege_games set status='cancelled',last_action='series_matchmaking_expired',updated_at=v_now
   where status='waiting' and game_mode='series'
     and greatest(coalesce(updated_at,created_at),created_at)<v_now-interval '30 minutes';
  ...
  if r.id is not null then
    update public.word_siege_games
       set player_two_id=v_uid,player_two_rack=substring(r.bag from 1 for 7),bag=substring(r.bag from 8),
           status='playing',current_player_id=r.player_one_id,turn_started_at=v_now,
           turn_deadline=v_now+make_interval(mins=>v_minutes),player_one_missed_turns=0,player_two_missed_turns=0,
  ...
  insert into public.word_siege_games(
    player_one_id,language,game_mode,turn_duration_minutes,turn_duration_hours,board,bag,player_one_rack,last_action
  ) values(
    v_uid,v_language,'series',v_minutes,12,private.word_siege_new_board_v1(),v_bag,v_rack,'series_waiting_for_opponent'
  ) returning * into r;
  return r;
end
$$;
...
begin
  ...
  if r.status='playing' and r.current_player_id is not null then
    update public.word_siege_games set
...
begin
  update public.word_siege_games set
    player_one_missed_turns=case when p_actor=player_one_id then 0 else player_one_missed_turns end,
    player_two_missed_turns=case when p_actor=player_two_id then 0 else player_two_missed_turns end
  where id=p_game_id and game_mode='series' returning * into r;
...
begin
  ...
  if v_missed>=3 then
    update public.word_siege_games set
      player_one_missed_turns=case when v_loser=player_one_id then 3 else player_one_missed_turns end,
      ...
      last_action='series_timeout_loss',last_action_player_id=v_loser,last_move_at=v_now,
      turn_started_at=null,turn_deadline=null,finished_at=v_now,updated_at=v_now
    where id=r.id returning * into r;
    return r;
  end if;

  update public.word_siege_games set
    player_one_missed_turns=case when v_loser=player_one_id then v_missed else player_one_missed_turns end,
    player_two_missed_turns=case when v_loser=player_two_id then v_missed else player_two_missed_turns end,
    current_player_id=v_winner,consecutive_passes=0,move_count=move_count+1,
    last_action='series_auto_pass',last_action_player_id=v_loser,last_move_at=v_now,
    turn_started_at=v_now,turn_deadline=v_now+make_interval(mins=>coalesce(turn_duration_minutes,5)),updated_at=v_now
  where id=r.id returning * into r;
  return r;
end
$$;
...
-- Keep current authoritative action pipeline; only reset a Series player's miss streak after a real action.
create or replace function public.submit_word_siege_move_v1(
  p_game_id uuid,p_placements jsonb,p_horizontal boolean default true
)
returns public.word_siege_games
language plpgsql
security definer
...
begin
  ...
  select m.move_number into v_existing_move_number from public.word_siege_moves m
   where m.game_id=p_game_id and m.player_id=v_uid and m.request_fingerprint=v_fingerprint
   order by m.id desc limit 1;
  if v_existing_move_number is not null and r.move_count=v_existing_move_number and r.last_action_player_id=v_uid then return r; end if;
  ...
  if r.game_mode='series' then r:=private.word_siege_series_reset_actor_v1(r.id,v_uid); end if;
...
set search_path='pg_catalog','public','private','pg_temp'
as $$
declare r public.word_siege_games; v_uid uuid:=auth.uid();
begin
  if v_uid is null then raise exception 'word_siege_unauthorized'; end if;
  r:=private.word_siege_prepare_turn_v2(p_game_id);
  if r.status<>'playing' then return r; end if;
...
grant execute on function public.submit_word_siege_move_v1(uuid,jsonb,boolean) to authenticated,service_role;
grant execute on function public.pass_word_siege_turn_v1(uuid) to authenticated,service_role;
grant execute on function public.exchange_word_siege_tiles_v1(uuid,jsonb) to authenticated,service_role;
...
create index if not exists word_siege_series_invites_receiver_v1_idx
  on public.word_siege_series_invites(receiver_id,status,expires_at);
...
begin
  ...
  if exists(select 1 from public.word_siege_series_invites i where i.status='pending' and i.expires_at>=now()
      and((i.sender_id=v_uid and i.receiver_id=p_friend_id) or(i.sender_id=p_friend_id and i.receiver_id=v_uid))) then raise exception 'word_siege_invite_pending'; end if;
