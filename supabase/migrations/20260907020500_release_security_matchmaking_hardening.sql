...
-- Closes anonymous SECURITY DEFINER state-mutation paths and enforces the approved
-- 15-second real-player matchmaking window before bot fallback.
...
begin
  ...
  if v_uid is distinct from r.host_id and v_uid is distinct from r.guest_id then raise exception 'not_participant'; end if;
  ...
  if r.status='sudden_death' then
    ...
    set status='finished',winner_id=case when timed_out_player=host_id then guest_id else host_id end,
        finished_at=now(),turn_deadline=null,last_event='turn_expired',last_event_player_id=timed_out_player
  ...
  set current_player_id=case when timed_out_player=host_id then guest_id else host_id end,
      turn_deadline=now()+interval '45 seconds',last_event='turn_expired',last_event_player_id=timed_out_player,
...
begin
  ...
  if r.status not in ('playing','final','sudden_death') then return r; end if;
  remaining_ms:=case when r.turn_deadline is null then null else greatest(0,(extract(epoch from (r.turn_deadline-now()))*1000)::bigint) end;
  update public.game_rooms
  set status='paused',paused_at=now(),paused_by=v_uid,paused_turn_player_id=current_player_id,
      paused_turn_remaining_ms=remaining_ms,turn_deadline=null,last_event='private_room_paused',last_event_player_id=v_uid
  where id=r.id returning * into r;
  return r;
end;
$$;
...
begin
  ...
  if r.room_mode<>'private' then raise exception 'private_room_required'; end if;
  if r.status<>'paused' then return r; end if;
  resume_status:=case when r.final_moves_remaining>0 then 'final' else 'playing' end;
  resume_deadline:=now()+make_interval(secs=>greatest(5,coalesce(r.paused_turn_remaining_ms,45000)/1000)::int);
  update public.game_rooms
  set status=resume_status,current_player_id=coalesce(paused_turn_player_id,current_player_id),turn_deadline=resume_deadline,
      paused_at=null,paused_by=null,paused_turn_player_id=null,paused_turn_remaining_ms=null,
      last_event='private_room_resumed',last_event_player_id=v_uid
  where id=r.id returning * into r;
  return r;
end;
$$;
  ...
  if q.status='waiting' then
    ...
    if q.queued_at<=now()-interval '15 seconds' then
      ...
      insert into public.game_rooms(
        ...
        generated_code,v_uid,'playing',v_uid,public.sonharf_turn_deadline(q.game_mode),q.language,true,
        case when q.language='tr' then 'KelimeBot' else 'WordBot' end,false,'bot',q.game_mode,now()
      ) returning * into r;
      ...
      if not found then
        ...
        select gr.* into r from public.matchmaking_queue mq left join public.game_rooms gr on gr.id=mq.room_id where mq.user_id=v_uid;
        return r;
      end if;
