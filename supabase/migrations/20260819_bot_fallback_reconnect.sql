...
-- Allow a temporary paused state regardless of the historical constraint name.
do $$
declare c record;
begin
  for c in
...
alter table public.game_rooms
  add constraint game_rooms_status_final_check
...
-- If an older trivia selector picks an already-used question, transparently replace it
-- with a still-unused question in the same language. This prevents repeats in a match.
...
create trigger trivia_no_repeat_in_room
before insert on public.trivia_rounds
for each row execute function public.prevent_room_trivia_repeat();

-- After 10 seconds with no compatible real opponent, create a server-controlled bot match.
      ...
      loop
        ...
        begin
          insert into public.game_rooms(
            code,host_id,guest_id,status,current_player_id,turn_deadline,language,
            is_bot,bot_name,host_last_seen,guest_last_seen,bot_turn
          ) values (
            generated_code,auth.uid(),null,'playing',auth.uid(),now()+interval '45 seconds',q.language,
            true,case when q.language='tr' then 'KelimeBot' else 'WordBot' end,now(),now(),false
          ) returning * into r;
          exit;
...
-- A deterministic server-side bot move. The client may request the move, but cannot
-- choose the word, score, timing outcome or result.
...
begin
  ...
  from public.game_words where room_id=r.id order by id desc limit 1;
  expected_first:=case when previous_word is null then null else right(previous_word,1) end;

  select d.normalized_word into clean_word
  from public.dictionary_words d
  where d.language=r.language and d.active
    and (expected_first is null or left(d.normalized_word,1)=expected_first)
  ...
  if clean_word is null then
    if r.status='sudden_death' then
      update public.game_rooms set status='finished',winner_id=host_id,finished_at=now(),turn_deadline=null,
        bot_turn=false,last_event='bot_failed' where id=r.id returning * into r;
      return r;
    end if;
    update public.game_rooms set guest_score=guest_score-1,guest_streak=0,current_player_id=host_id,
      bot_turn=false,turn_deadline=now()+interval '45 seconds',last_event='bot_failed'
    where id=r.id returning * into r;
    return r;
  end if;

  insert into public.game_words(room_id,player_id,word,normalized_word,is_bot)
  values(r.id,null,clean_word,clean_word,true);
  ...
  set guest_score=guest_score+add_points,
      ...
      valid_word_count=next_valid_count,
      round_word_count=next_valid_count%10,
  ...
  if next_valid_count>=30 and r.status='playing' then
    ...
    set status=case when host_score=guest_score then 'sudden_death' else 'finished' end,
        winner_id=case when host_score>guest_score then host_id when guest_score>host_score then null else null end,
        finished_at=case when host_score=guest_score then null else now() end,
        bot_turn=case when host_score=guest_score then false else bot_turn end,
        current_player_id=case when host_score=guest_score then host_id else current_player_id end,
...
-- Bot gets one server-side attempt per trivia round after the 3-second reading lock.
...
begin
  ...
  if not bot_correct then return r; end if;
  ...
  if tr.milestone>=30 then
    ...
    update public.game_rooms set status='playing',current_player_id=host_id,bot_turn=false,
      turn_deadline=now()+interval '45 seconds',last_event='bot_quiz_won'
    where id=r.id returning * into r;
  end if;
  return r;
end $$;
...
-- Keep bot_turn synchronized after a valid human word in bot matches.
...
-- Heartbeat + reconnect. A real opponent gets a 60-second grace window.
...
begin
  select * into r from public.game_rooms where id=p_room_id for update;
  if r.id is null or me not in (r.host_id,r.guest_id) then raise exception 'not_participant'; end if;
  if r.is_bot then return r; end if;

  if me=r.host_id then
    update public.game_rooms set host_last_seen=now() where id=r.id;
    opponent:=r.guest_id; opponent_seen:=r.guest_last_seen;
  else
    update public.game_rooms set guest_last_seen=now() where id=r.id;
    opponent:=r.host_id; opponent_seen:=r.host_last_seen;
  end if;
  ...
  if r.status='paused' then
    if r.disconnected_player_id=me and r.reconnect_deadline>=now() then
      update public.game_rooms
      set status=coalesce(status_before_pause,'playing'),disconnected_player_id=null,reconnect_deadline=null,
          status_before_pause=null,turn_deadline=case when coalesce(status_before_pause,'playing')='quiz' then null else now()+interval '45 seconds' end,
          last_event='player_reconnected'
      where id=r.id returning * into r;
      return r;
    elsif r.reconnect_deadline<now() then
      update public.game_rooms
      set status='finished',winner_id=case when disconnected_player_id=host_id then guest_id else host_id end,
  ...
  if r.status in ('playing','quiz','final','sudden_death') and opponent is not null
     and opponent_seen < now()-interval '15 seconds' then
    update public.game_rooms
    set status_before_pause=status,status='paused',disconnected_player_id=opponent,
