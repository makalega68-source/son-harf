-- Son Harf bot v2: a thinking opponent instead of a random word picker.
--
-- * Strategy: every candidate is scored by how hard its last letter is to answer (few dictionary
--   words start with it) and by the long-word bonus it earns, then blended with noise according
--   to the bot's current skill.
-- * Adaptive skill: base skill by tier, lower for beginners, and a rubber band on the round and
--   match score so games stay close and exciting in both directions.
-- * Human-like slips: now and then (more often when it is comfortably ahead) the bot cannot find
--   a word, loses a point and passes the turn, exactly like a player's failed attempt.
-- * First words of a round start from a random common letter instead of always "a".
-- The round/score bookkeeping after a word is unchanged from bot_take_turn_normal_v1.

begin;

create table if not exists public.sonharf_letter_openings (
  language text not null,
  letter text not null,
  word_count integer not null,
  refreshed_at timestamptz not null default now(),
  primary key (language, letter)
);

alter table public.sonharf_letter_openings enable row level security;
revoke all on table public.sonharf_letter_openings from public, anon, authenticated;

create or replace function private.sonharf_refresh_letter_openings_v1()
returns void
language sql
security definer
set search_path = pg_catalog, public
as $$
  insert into public.sonharf_letter_openings(language, letter, word_count, refreshed_at)
  select d.language, left(d.normalized_word, 1), count(*)::integer, now()
  from public.dictionary_words d
  where d.active and d.game_allowed and not d.is_abbreviation and not d.is_proper_noun
  group by d.language, left(d.normalized_word, 1)
  on conflict (language, letter) do update
    set word_count = excluded.word_count,
        refreshed_at = excluded.refreshed_at;
$$;

revoke all on function private.sonharf_refresh_letter_openings_v1() from public, anon, authenticated;

select private.sonharf_refresh_letter_openings_v1();
...
begin
  ...
  v_skill := v_skill
    - greatest(-12, least(12, v_lead)) * .025
  ...
  if r.status <> 'sudden_death' and r.host_round_words < 10 then
    v_miss := .015 + (1 - v_skill) * .07 + case when v_lead >= 8 then .05 else 0 end;
  ...
  from scored s
  order by v_skill * (.6 * s.trap + .4 * s.length_value)
    + (1 - v_skill) * .9 * random()
    + .12 * random() desc
  limit 1;
