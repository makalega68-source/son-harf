...
class WordSiegePracticeBoardContractTest {
    @Test fun `practice board uses readable pannable close and fit modes`() {
        ...
        assertTrue(board.contains("detectTapGestures("))
        assertTrue(board.contains("onDoubleTap = { toggleMode() }"))
        assertTrue(board.contains("onTap = { tap ->"))
        assertTrue(board.contains("val nextMode = mode.toggle()"))
        ...
        assertFalse(board.contains(".clickable(enabled = enabled && (cell.letter == null || pending)"))
