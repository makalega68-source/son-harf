...
class WordSiegePanAreaContractTest {
    @Test fun boardIsLargeBoundedTwoDimensionalAndCentered() {
        ...
        assertTrue(pan.contains("onDoubleClick = onDoubleClick"))
        ...
        assertTrue(viewport.contains("value.coerceIn(viewportPx - renderedBoardPx, 0f)"))
        assertTrue(pan.contains("centerOn(WordSiegeBoardSpec.CenterIndex)"))
    ...
    @Test fun boardKeepsReadableOwnershipAndTapPlacementContract() {
        ...
        assertTrue(pan.contains("onClick = { if (canPlace) onClick() }"))
        assertTrue(pan.contains("onDoubleClick = onDoubleClick"))
