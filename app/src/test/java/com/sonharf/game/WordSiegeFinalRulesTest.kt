...
class WordSiegeFinalRulesTest {
    ...
    @Test fun botAndHumanUseSameValidationAndSharedZoneBoard() {
        ...
        assertTrue(board.contains("letter != null -> territory"))
        assertTrue(board.contains("else -> PracticeSiegeEmpty"))
        assertFalse(board.contains("owner != 0 -> territory.copy"))
        assertFalse(board.contains("fortress = WordSiegeZoneRules.isFortress(zoneId)"))
