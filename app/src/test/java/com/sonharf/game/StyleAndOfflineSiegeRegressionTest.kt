...
class StyleAndOfflineSiegeRegressionTest {
    ...
    fun practiceUsesPersistentCanonicalDictionaryWithoutDivergentFallback() {
        ...
        assertFalse(practice.contains("ANA SÖZLÜK"))
        assertFalse(practice.contains("MAIN DICTIONARY"))
        assertTrue(practice.contains("Sözlük hazır"))
    ...
    fun styleShowsBackendProfileFramesAndNeverOverlaysBrokenImage() {
        ...
        assertTrue(frames.contains("PurchasedFrameCatalog.OCEAN"))
        assertTrue(frames.contains("PurchasedFrameCatalog.ROSE"))
        assertTrue(frames.contains("PurchasedFrameCatalog.GOLDEN_AVATAR"))
