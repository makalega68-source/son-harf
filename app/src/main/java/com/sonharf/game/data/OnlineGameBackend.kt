...
class OnlineGameBackend(private val supabase: SupabaseClient = SupabaseProvider.client) {
    ...
    suspend fun pollRandomMatchmakingRoom(): GameRoomDto? {
        ...
        val queue = supabase.from("matchmaking_queue")
            .select { filter { eq("user_id", me) } }
            .decodeList<MatchmakingQueueDto>()
    ...
    suspend fun forfeit(roomId: String): GameRoomDto =
        supabase.postgrest.rpc("forfeit_room", buildJsonObject { put("p_room_id", roomId) }).decodeSingle()
