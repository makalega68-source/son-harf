...
internal object MascotAiChatService {
    ...
    private fun localFallback(request: MascotChatRequest): MascotChatResponse {
        ...
        val response = when {
            ...
            total == 0 ->
                MascotChatResponse(
                    reply = if (en) "Ready? Start simple and watch the final letter." else "Hazır mısın? Basit başla, son harfi takip et.",
                    mood = "happy",
                    animation = "greeting",
                    usedFallback = true,
                )
            ...
            streak >= 3 ->
                ...
                MascotChatResponse(
                    reply = if (en) "One good word is enough to start. Let's play." else "Başlamak için tek iyi kelime yeter. Hadi oynayalım.",
                    mood = "calm",
                    animation = "greeting",
                    usedFallback = true,
                )
