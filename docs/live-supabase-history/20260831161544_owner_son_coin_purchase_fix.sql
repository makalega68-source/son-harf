...
-- Live version: 20260831161544
-- Live name: owner_son_coin_purchase_fix
-- Live statement MD5: de99c49d4495bce975af9d58f6eff0cc

create or replace function public.buy_mascot_fruit_v1(p_fruit_id text, p_quantity integer default 1)
returns table(success boolean,fruit_id text,quantity integer,inventory_quantity integer,son_coin_spent integer,son_coin_balance integer)
language plpgsql security definer
set search_path = pg_catalog, public, pg_temp
as $$
declare
  v_uid uuid:=auth.uid(); v_price integer; v_magic boolean; v_cost integer; v_balance integer;
  v_qty integer:=greatest(1,least(coalesce(p_quantity,1),20)); v_inventory integer; v_owner_unlimited boolean:=false;
begin
  if v_uid is null then raise exception 'not_authenticated'; end if;
  select c.son_coin_price,c.is_magic into v_price,v_magic from public.mascot_fruit_catalog c where c.id=p_fruit_id and c.active=true;
  ...
  select exists(select 1 from public.owner_game_accounts o where o.user_id=v_uid and o.active and o.unlimited_son_coin) into v_owner_unlimited;
  if not v_owner_unlimited then
    ...
    values(v_uid,'owner_unlimited_son_coin_purchase','mascot_fruit',p_fruit_id,jsonb_build_object('quantity',v_qty,'normal_son_coin_cost',v_cost),'success');
  end if;
  ...
  return query select true,p_fruit_id,v_qty,v_inventory,case when v_owner_unlimited then 0 else v_cost end,v_balance;
end $$;
