...
-- Live version: 20260904083238
-- Live name: reward_center_v8
-- Live statement MD5: 435312e09b3ef3c193f11453aec886e4
...
update public.shop_items set trial_mode='minutes',trial_value=30 where id='frame_asset_gold' and active=true;
update public.shop_items set trial_mode='match',trial_value=1 where id='theme_monster_blue' and active=true;
create or replace function public.completed_store_match_count_v1(p_uid uuid)
returns integer language sql security invoker set search_path='' stable as $$
select ((select count(*) from public.game_rooms g where g.status='finished' and g.stats_applied and (g.host_id=p_uid or g.guest_id=p_uid)) + (select count(*) from public.word_arena_rooms a where a.status='finished' and a.result_applied and (a.host_id=p_uid or a.guest_id=p_uid)))::integer
$$;
...
begin
 ...
 select count(*) filter(where reward_type='diamonds'), count(*) filter(where reward_type='trial') into v_ad_count,v_trial_count from public.rewarded_ad_claims where user_id=v_uid and reward_date=v_day;
 ...
 select baseline_match_count into v_baseline from public.piggy_banks where user_id=v_uid for update;
 ...
 select * into v_trial from public.style_trials where user_id=v_uid and ended_at is null and (expires_at is null or expires_at>now()) and (matches_remaining is null or matches_remaining>0) order by started_at desc limit 1;
 return jsonb_build_object('coin_ads_used',coalesce(v_ad_count,0),'coin_ads_limit',3,'coin_per_ad',10,'trial_ads_used',coalesce(v_trial_count,0),'trial_ads_limit',1,'trial_item_id',v_trial.item_id,'trial_mode',v_trial.mode,'trial_matches_remaining',v_trial.matches_remaining,'trial_expires_at',v_trial.expires_at,'piggy_tier',v_tier,'piggy_bonus_sc',v_bonus,'piggy_match_progress',least(v_delta,8),'piggy_match_target',8);
end
$$;
...
create or replace function public.claim_store_rewarded_ad_v1(p_reward_type text,p_ad_response_id text,p_trial_item_id text default null)
returns jsonb language plpgsql security definer set search_path='' as $$
declare
 v_uid uuid := auth.uid();
 v_day date := (timezone('utc',now()))::date;
...
begin
 if v_uid is null then raise exception 'unauthorized'; end if;
 select * into v_trial from public.style_trials where user_id=v_uid and ended_at is null and (expires_at is null or expires_at>now()) and (matches_remaining is null or matches_remaining>0) order by started_at desc limit 1 for update;
...
begin
 ...
 update public.piggy_banks set tier=0,bonus_sc=0,baseline_match_count=v_match_count,last_match_count=v_match_count,opened_at=now(),updated_at=now() where user_id=v_uid;
