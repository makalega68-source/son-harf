...
-- Live version: 20260831161225
-- Live name: admin_security_hardening_v3
-- Live statement MD5: 4e4936c026dc0b44fa50c8c95543a4a9
...
insert into public.owner_game_accounts(user_id,lifetime_vip,unlimited_diamonds,unlimited_son_coin,active,notes,updated_by)
select u.id,true,true,true,true,'Kurucu rekabet hesabı',u.id
from auth.users u where lower(u.email)='makalega68@gmail.com'
on conflict(user_id) do update set lifetime_vip=true,unlimited_diamonds=true,unlimited_son_coin=true,active=true,updated_at=now();
...
as $$
  select exists(
    select 1 from public.admin_users a join auth.users u on u.id=a.user_id
...
returns table(authorized boolean,admin_role text,lifetime_vip boolean,unlimited_diamonds boolean,unlimited_son_coin boolean)
language sql stable security definer
set search_path = pg_catalog, public, auth, pg_temp
as $$
  select public.is_admin(),case when public.is_admin() then coalesce(a.role,'') else '' end,
    coalesce(o.active and o.lifetime_vip,false),coalesce(o.active and o.unlimited_diamonds,false),coalesce(o.active and o.unlimited_son_coin,false)
...
alter table public.admin_audit_log add column if not exists outcome text not null default 'success', add column if not exists error_text text;

create or replace function public.enforce_chat_admin_control_v1()
returns trigger language plpgsql security definer
set search_path = pg_catalog, public, pg_temp
as $$ begin
  if not public.sonharf_config_enabled('chat_enabled',true) and not public.is_admin() then raise exception 'chat_disabled'; end if;
  return new;
end $$;
...
create trigger enforce_chat_admin_control_v1 before insert on public.chat_messages for each row execute function public.enforce_chat_admin_control_v1();
drop trigger if exists enforce_word_siege_chat_admin_control_v1 on public.word_siege_messages;
create trigger enforce_word_siege_chat_admin_control_v1 before insert on public.word_siege_messages for each row execute function public.enforce_chat_admin_control_v1();
...
returns public.word_siege_games language plpgsql security definer
...
as $$ begin
  ...
  if not public.sonharf_config_enabled('matchmaking_enabled',true) and not public.is_admin() then raise exception 'matchmaking_disabled'; end if;
...
set search_path = pg_catalog, public, pg_temp
as $$
declare v_action text:=lower(trim(coalesce(p_action,''))); v_count bigint:=0; v_total bigint:=0; v_detected bigint:=0; v_result jsonb;
begin
  ...
  if v_action in ('stale_rooms','all') then
    select count(*) into v_detected from public.game_rooms r where r.status in ('playing','quiz','final','sudden_death','paused') and greatest(coalesce(r.host_last_seen_at,r.created_at),coalesce(r.guest_last_seen_at,r.created_at)) < now()-interval '5 minutes';
  end if;
  ...
  values(auth.uid(),'repair_'||v_action,'system',v_action,jsonb_build_object('affected',v_total,'stale_rooms_detected',v_detected,'maintenance',v_result),'success');
  return jsonb_build_object('success',true,'action',v_action,'affected',v_total,'stale_rooms_detected',v_detected,'maintenance',v_result);
end $$;
