...
-- Live version: 20260904083637
-- Live name: season_style_equip_hardening
-- Live statement MD5: ecc32b097486e24a34b46ce927fd872c

delete from public.season_store_rewards where reward_key='frame_asset_blue_season';
delete from public.shop_items where id='frame_asset_blue_season';
insert into public.season_store_rewards(season_id,level,track,reward_type,reward_key,amount,sort_order)
select s.id,3,'premium','son_coin','',125,40 from public.competitive_seasons s where s.starts_at<=now() and s.ends_at>now() on conflict do nothing;
...
begin
  ...
  select exists(select 1 from public.user_inventory i where i.user_id=v_uid and i.item_id=p_item_id) into v_owned;
  if not v_owned then raise exception 'not_owned'; end if;
  select * into v_item from public.shop_items where id=p_item_id and (active=true or rarity in ('SEASON','EVENT'));
