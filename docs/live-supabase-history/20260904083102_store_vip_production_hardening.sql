...
-- Live version: 20260904083102
-- Live name: store_vip_production_hardening
-- Live statement MD5: 90a87a9481c9313b9cc3760b71fba46f
...
create policy store_catalog_config_read on public.store_catalog_config for select to authenticated using (true);
...
insert into public.store_catalog_config(product_id,badge_tr,badge_en,sort_order) values
 ('vip_monthly',null,null,10),('vip_yearly','EN AVANTAJLI','BEST VALUE',11),('coins_500','MINI','MINI',20),('coins_1500','EN POPÜLER','POPULAR',21),('coins_3500','EN İYİ DEĞER','BEST VALUE',22),('coins_8000','MEGA','MEGA',23),('starter_style_pack',null,null,30),('premium_style_pack',null,null,31),('season_pack',null,null,32),('vip_welcome_pack',null,null,33)
...
create index if not exists store_entitlements_user_active_idx on public.store_entitlements(user_id,status,expires_at);
...
create policy store_entitlements_read_own on public.store_entitlements for select to authenticated using ((select auth.uid())=user_id);
...
create unique index if not exists diamond_ledger_purchase_reason_unique on public.diamond_ledger(user_id,reason) where reason like 'google_play_purchase:%';
create or replace function public.prevent_son_coin_ledger_mutation_v1()
returns trigger language plpgsql set search_path=public,pg_temp as $$ begin raise exception 'son_coin_ledger_is_immutable'; end $$;
drop trigger if exists trg_son_coin_ledger_immutable_update on public.diamond_ledger;
create trigger trg_son_coin_ledger_immutable_update before update or delete on public.diamond_ledger for each row execute function public.prevent_son_coin_ledger_mutation_v1();
...
alter table public.shop_items add constraint shop_items_rarity_check check (rarity in ('STANDARD','RARE','EPIC','LEGENDARY','SEASON','EVENT','VIP'));
...
create policy style_trials_read_own on public.style_trials for select to authenticated using ((select auth.uid())=user_id);
...
create policy social_preferences_read_own on public.player_social_preferences for select to authenticated using ((select auth.uid())=user_id);
create policy social_preferences_write_own on public.player_social_preferences for all to authenticated using ((select auth.uid())=user_id) with check ((select auth.uid())=user_id);
...
create policy relationship_marks_own on public.player_relationship_marks for all to authenticated using ((select auth.uid())=user_id) with check ((select auth.uid())=user_id);
...
create index if not exists match_analysis_user_completed_idx on public.match_analysis_snapshots(user_id,completed_at desc);
...
create policy match_analysis_read_own on public.match_analysis_snapshots for select to authenticated using ((select auth.uid())=user_id);
...
create policy piggy_read_own on public.piggy_banks for select to authenticated using ((select auth.uid())=user_id);
grant select on public.piggy_banks to authenticated;

create or replace function public.open_piggy_bank_v1()
returns jsonb language plpgsql security definer set search_path=public,pg_temp as $$
declare v_uid uuid:=auth.uid(); v_bonus integer; v_balance integer;
begin
 ...
 insert into public.diamond_ledger(user_id,delta,reason) values(v_uid,v_bonus,'piggy_open:'||extract(epoch from now())::bigint::text);
...
returns jsonb language plpgsql security definer set search_path=public,pg_temp as $$
begin if auth.uid() is null then raise exception 'unauthorized'; end if; return jsonb_build_object('success',false,'disabled',true,'reason','replaced_by_deterministic_piggy'); end $$;

create or replace function public.claim_optional_reward_v7(p_reward_type text,p_ad_response_id text default null)
returns jsonb language plpgsql security definer set search_path=public,pg_temp as $$
declare v_uid uuid:=auth.uid(); v_day date:=(timezone('utc',now()))::date; v_used int; v_limit int; v_proof text:=nullif(trim(p_ad_response_id),''); v_amount int; v_balance int; v_trial text;
begin
 if v_uid is null then raise exception 'unauthorized'; end if;
 ...
 if p_reward_type='diamonds' then
  ...
  insert into public.diamond_ledger(user_id,delta,reason) values(v_uid,v_amount,'rewarded_ad:'||v_proof);
  insert into public.rewarded_ad_claims(user_id,reward_type,ad_response_id,reward_date,diamonds_awarded) values(v_uid,p_reward_type,v_proof,v_day,v_amount);
  return jsonb_build_object('success',true,'reward_type','diamonds','diamonds_awarded',v_amount,'diamonds',v_balance);
 end if;
 select id into v_trial from public.shop_items where active=true and trial_mode is not null order by sort_order,id limit 1;
 if v_trial is null then raise exception 'trial_item_unavailable'; end if;
 insert into public.rewarded_ad_claims(user_id,reward_type,ad_response_id,reward_date,trial_item_id) values(v_uid,p_reward_type,v_proof,v_day,v_trial);
...
returns jsonb language plpgsql security definer set search_path=public,pg_temp as $$
declare v_uid uuid:=auth.uid(); v_vip boolean;
begin
 if v_uid is null then raise exception 'unauthorized'; end if;
 select coalesce(is_vip,false) into v_vip from public.profiles where id=v_uid;
 return jsonb_build_object('is_vip',coalesce(v_vip,false),'daily_jokers_claimed',true,'freezer_count',0,'swap_count',0,'hint_count',0,'streak_shield_count',0,'xp_multiplier',1,'diamond_multiplier',1,'rewarded_ad_bypass',false,'used_words_access',true,'direct_messages_access',true,'ranked_live_assist',false,'post_match_analysis',coalesce(v_vip,false),'saved_friend_list',coalesce(v_vip,false),'private_rooms',coalesce(v_vip,false));
end $$;
...
create table if not exists public.store_analytics_events (
  ...
  check (event_name in ('store_view','product_view','preview_start','checkout_start','purchase_success','purchase_cancel','purchase_failure','purchase_pending','restore','equip','vip_start','vip_renew','vip_cancel','vip_expire','season_upgrade','rewarded_ad_complete','piggy_open'))
);
...
begin
 if v_uid is null then raise exception 'unauthorized'; end if;
 if p_event_name not in ('store_view','product_view','preview_start','checkout_start','purchase_success','purchase_cancel','purchase_failure','purchase_pending','restore','equip','vip_start','vip_renew','vip_cancel','vip_expire','season_upgrade','rewarded_ad_complete','piggy_open') then raise exception 'invalid_event'; end if;
 p_metadata:=coalesce(p_metadata,'{}'::jsonb)-array['purchaseToken','purchase_token','orderId','order_id','email','phone','message'];
 insert into public.store_analytics_events(user_id,event_name,product_id,metadata) values(v_uid,p_event_name,nullif(trim(p_product_id),''),p_metadata);
end $$;
