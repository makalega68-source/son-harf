...
-- Live version: 20260904083323
-- Live name: word_siege_area_score_authority
-- Live statement MD5: 840ddd901b45c51eda90302f28d70f76

create or replace function private.word_siege_sync_area_scores_v1()
returns trigger language plpgsql security invoker set search_path=pg_catalog,public,private,pg_temp as $$
declare
  v_one integer := 0;
  v_two integer := 0;
begin
...
create trigger trg_word_siege_sync_area_scores_v1 before insert or update of board on public.word_siege_games for each row execute function private.word_siege_sync_area_scores_v1();
update public.word_siege_games set board=board where board is not null;

create or replace function private.word_siege_preview_move_v1(p_game_id uuid,p_placements jsonb,p_horizontal boolean)
...
begin
  ...
  v_main_cells := private.word_siege_collect_cells_v1(v_board,p_placements,v_rack,v_anchor,case when p_horizontal then 1 else 9 end);
  ...
  foreach v_index in array v_indices loop
    v_cross_cells := private.word_siege_collect_cells_v1(v_board,p_placements,v_rack,v_index,case when p_horizontal then 9 else 1 end);
  ...
  select coalesce(array_agg(i order by i),array[]::integer[]) into v_new_area from generate_series(0,80) i where coalesce((r.board -> i ->> 'owner')::integer,0)<>v_owner and coalesce((v_board -> i ->> 'owner')::integer,0)=v_owner;
