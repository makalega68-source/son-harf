...
-- Live version: 20260904083343
-- Live name: vip_social_authority
-- Live statement MD5: ce8ad8375a3fd0c955d831a7059b509f
...
create or replace function public.set_vip_relationship_mark_v1(p_other_user_id uuid,p_favorite boolean default false,p_arch_rival boolean default false)
...
begin
  ...
  select exists(
    select 1 from public.game_rooms g where g.status='finished' and coalesce(g.is_bot,false)=false and ((g.host_id=v_uid and g.guest_id=p_other_user_id) or (g.host_id=p_other_user_id and g.guest_id=v_uid))
    union all
    select 1 from public.word_arena_rooms a where a.status='finished' and a.result_applied and ((a.host_id=v_uid and a.guest_id=p_other_user_id) or (a.host_id=p_other_user_id and a.guest_id=v_uid))
    union all
    select 1 from public.word_siege_games s where s.status='finished' and ((s.player_one_id=v_uid and s.player_two_id=p_other_user_id) or (s.player_one_id=p_other_user_id and s.player_two_id=v_uid))
    limit 1
  ) into v_has_match;
  ...
  on conflict(user_id,other_user_id) do update set favorite=excluded.favorite,arch_rival=excluded.arch_rival,updated_at=now();
  return jsonb_build_object('success',true,'other_user_id',p_other_user_id,'favorite',p_favorite,'arch_rival',p_arch_rival);
end $$;
...
returns table(other_user_id uuid,display_name text,favorite boolean,arch_rival boolean,presence_status text,last_seen_at timestamptz,is_friend boolean,can_invite boolean)
...
begin
  ...
  if not v_vip then raise exception 'vip_required'; end if;
  return query
  select m.other_user_id,p.display_name,m.favorite,m.arch_rival,
    ...
    (public.are_friends(v_uid,m.other_user_id) and not exists(select 1 from public.user_blocks b where (b.blocker_id=v_uid and b.blocked_id=m.other_user_id) or (b.blocker_id=m.other_user_id and b.blocked_id=v_uid)))
