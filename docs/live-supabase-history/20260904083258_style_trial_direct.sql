...
-- Live version: 20260904083258
-- Live name: style_trial_direct
-- Live statement MD5: 85a400fd59d26323ffacc7f31c212395
...
begin
  ...
  select * into v_item from public.shop_items where id=nullif(trim(p_item_id),'') and active=true and trial_mode in ('match','minutes') and coalesce(trial_value,0)>0;
  if not found then raise exception 'trial_item_unavailable'; end if;
  if exists(select 1 from public.user_inventory where user_id=v_uid and item_id=v_item.id) then raise exception 'already_owned'; end if;
  insert into public.style_trial_daily_claims(user_id,item_id,claim_date) values(v_uid,v_item.id,v_date) on conflict do nothing;
  ...
  if v_item.trial_mode='minutes' then v_expires := now()+make_interval(mins=>least(v_item.trial_value,30)); v_matches := null; else v_expires := null; v_matches := least(v_item.trial_value,1); end if;
  update public.style_trials set ended_at=now() where user_id=v_uid and ended_at is null;
  insert into public.style_trials(user_id,item_id,mode,matches_remaining,expires_at,match_count_at_start) values(v_uid,v_item.id,v_item.trial_mode,v_matches,v_expires,v_match_count);
  return jsonb_build_object('success',true,'trial_item_id',v_item.id,'trial_mode',v_item.trial_mode,'trial_matches_remaining',v_matches,'trial_expires_at',v_expires);
end $$;
...
begin
  if v_uid is null then raise exception 'unauthorized'; end if;
  select * into v_trial from public.style_trials where user_id=v_uid and ended_at is null order by started_at desc limit 1 for update;
  if not found then raise exception 'trial_not_active'; end if;
  if v_trial.mode='minutes' then
    if v_trial.expires_at is null or v_trial.expires_at<=now() then update public.style_trials set ended_at=now() where id=v_trial.id; raise exception 'trial_expired'; end if;
  else
    v_current_matches := public.completed_store_match_count_v1(v_uid);
    if v_trial.match_count_at_start is null or v_current_matches-v_trial.match_count_at_start>=coalesce(v_trial.matches_remaining,1) then update public.style_trials set ended_at=now() where id=v_trial.id; raise exception 'trial_expired'; end if;
  end if;
